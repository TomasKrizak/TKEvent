#include "build_event_3D.hh"

using namespace std;

Event_3D::Event_3D(int _run_number ,int _event_number)
{
	event_number = _event_number;
	run_number = _run_number;
}

int Event_3D::get_run_number()
{
	return run_number;
}

int Event_3D::get_event_number()
{	
	return event_number;
}

int Event_3D::get_track_side(int index)
{
	return rec_tracks_side.at(index);
}

double Event_3D::get_track_a(int index)
{
	return rec_tracks_a.at(index);
}

double Event_3D::get_track_b(int index)
{
	return rec_tracks_b.at(index);
}

double Event_3D::get_track_c(int index)
{
	return rec_tracks_c.at(index);
}

double Event_3D::get_track_d(int index)
{
	return rec_tracks_d.at(index);
}

int Event_3D::get_no_tracks()
{
...
void Event_3D::add_event(snemo::datamodel::raw_event_data red)
{
...
void Event_3D::add_calo_hit(const snemo::datamodel::calo_digitized_hit & red_calo_hit)
{
	...
	hit_is_HT.push_back(red_calo_hit.is_high_threshold());
}

void Event_3D::add_calo_hit(int _hit_om_num, bool _hit_is_HT, int64_t calo_tdc, int16_t peak_cell)
{
...
void Event_3D::add_calo_hit(int cell_side, int cell_wall, int cell_column, int cell_row, bool is_HT, int64_t calo_tdc, int16_t peak_cell)
{
	add_calo_hit(SWCR_to_OMnum(cell_side, cell_wall, cell_column, cell_row), is_HT, calo_tdc, peak_cell);
}

void Event_3D::add_tracker_hit(const snemo::datamodel::tracker_digitized_hit & red_tracker_hit)
{
...
void Event_3D::add_tracker_hit(int _hit_cell_num, double _hit_height, double _hit_radius)
{
...
void Event_3D::add_tracker_hit(int hit_cell_side, int hit_cell_row, int hit_cell_layer, double hit_height, double hit_radius)
{
	add_tracker_hit(SRL_to_Cellnum(hit_cell_side, hit_cell_row, hit_cell_layer), hit_height, hit_radius);
}

void Event_3D::reconstruct_track()
{
...
void Event_3D::make_top_projection()
{
	gROOT->SetBatch(true);
...
double Event_3D::get_tracker_hit_height(const snemo::datamodel::tracker_digitized_hit::gg_times & gg_timestamps)
{
	const int64_t anode_tdc = gg_timestamps.get_anode_time(0).get_ticks();
...
// drift model
double Event_3D::drift_time_to_radius(double time_, string type) 
{
	double r = tc_radius;
	if(type == "Manu")
	{
...
void Event_3D::SRL_to_xy(int cell_side, int cell_row, int cell_layer, double position_xy[])
{
...
void Event_3D::Cellnum_to_xy(int cell_num, double position_xy[])
{	
	int SRL[3];
...
int Event_3D::SRL_to_Cellnum(int cell_side, int cell_row, int cell_layer)
{
...
void Event_3D::Cellnum_to_SRL(int cell_num, int SRL[])
{
...
int Event_3D::SWCR_to_OMnum(int om_side, int om_wall, int om_column, int om_row)
{
	// auto detect MW
...
void Event_3D::OMnum_to_SWCR(int om_num, int OM[])
{
	//mainwall IT
	if(om_num < 260) 
	{
...
void Event_3D::OMnum_to_xyz(int om_num, double position_xyz[])
{
	int SWCR[4];
...
// warning: dimensions are approximately taken from Falaise and might not be exactly right
void Event_3D::SWCR_to_xyz(int om_side, int om_wall, int om_column, int om_row, double position_xyz[])
{
