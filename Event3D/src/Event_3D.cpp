...
std::vector<int> Event_3D::get_hit_om_num_v()
{
	return hit_om_num;
}

std::vector<int64_t> Event_3D::get_hit_om_tdc_v()
{
	return hit_om_tdc;
}	
...
void Event_3D::print()
{
	...
	for (int i = 0; i < hit_om_num.size(); i++)
	{
		std::cout << "OM "       << hit_om_num.at(i) 
		     	  << ", is HT: " << hit_is_HT.at(i) 
		     	  << ", tdc: "   << hit_om_tdc.at(i)
		     	  << ", pcell: " << hit_om_peak_cell.at(i) << std::endl;
	}
	
	for (int i = 0; i < hit_cell_num.size(); i++)
	{
		std::cout << "Cell: "    << hit_cell_num.at(i) 
		     	  << ", h = " << hit_height.at(i) << " mm"
		     	  << ", r = "   << hit_radius.at(i) << " mm" << std::endl;
	}
}

void Event_3D::print_tracks()
{
	std::cout << "*** Run: " << run_number << ", event: " << event_number << " ***" << std::endl;
...
double Event_3D::drift_time_to_radius(double time_, string type) 
{
	...
	if(type == "Manu")
	{
		...
		const double t_usec = time_ / 1000.0;
